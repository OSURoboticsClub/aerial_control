...
R ControllerPipeline<R>::run(const AttitudeEstimate& estimate, const SP& input, C& tail) {
  return tail.run(estimate, input);
}

template <typename R> template <typename SP, typename C, typename... Cs>
R ControllerPipeline<R>::run(const AttitudeEstimate& estimate, const SP& input, C& head, Cs&... controllers) {
