...
AngularAccelerationSetpoint AngularVelocityController::run(const AttitudeEstimate& estimate, const AngularVelocitySetpoint& input) {
  ...
  // Run PID controllers
  float rollAccSp = rollVelPid.calculate(rollVelSp, estimate.rollVel, unit_config::DT);
  float pitchAccSp = pitchVelPid.calculate(pitchVelSp, estimate.pitchVel, unit_config::DT);
  float yawAccSp = yawVelPid.calculate(input.yawVel, estimate.yawVel, unit_config::DT);
